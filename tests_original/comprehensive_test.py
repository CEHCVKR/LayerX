...
from a3_image_processing import dwt_decompose, dwt_reconstruct, psnr, get_capacity
...
# Helper wrapper functions for test compatibility
def embed_message(message: str, password: str, cover_path: str, stego_path: str):
    """Wrapper function: encrypt -> compress -> embed"""
    ...
    # Store salt/iv globally for extraction
    global _last_salt, _last_iv
    _last_salt, _last_iv = salt, iv
    return embed(payload, cover_path, stego_path)

def extract_message(password: str, stego_path: str) -> str:
    """Wrapper function: extract -> decompress -> decrypt"""
    ...
    # Use globally stored salt/iv
    global _last_salt, _last_iv
    return decrypt_message(decompressed, password, _last_salt, _last_iv)

def calculate_capacity(cover_path: str) -> int:
    """Wrapper function: get image capacity"""
    import cv2
    img = cv2.imread(cover_path, cv2.IMREAD_GRAYSCALE)
    return get_capacity(img.shape, 'dwt')
...
def test_compression_basic():
    ...
    message = "AAAABBBCCD".encode('utf-8')  # Compression works on bytes
...
def test_payload_format():
    ...
    message = "Test message for payload".encode('utf-8')  # Compression works on bytes
    tree, compressed = compress_huffman(message)
...
def test_embedding_basic():
    ...
    # Get capacity
    img = cv2.imread(cover_image, cv2.IMREAD_GRAYSCALE)
    capacity = get_capacity(img.shape, 'dwt')
    
    # Full pipeline: encrypt -> compress -> embed
    ciphertext, salt, iv = encrypt_message(message, password)
    compressed, tree = compress_huffman(ciphertext)
    payload = create_payload(ciphertext, tree, compressed)
    embed(payload, cover_image, stego_image)
    
    # Extract
    extracted_payload = extract(stego_image)
    msg_len, tree_ext, compressed_ext = parse_payload(extracted_payload)
    decompressed = decompress_huffman(compressed_ext, tree_ext)
    extracted = decrypt_message(decompressed, password, salt, iv)
