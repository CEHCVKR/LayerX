...
def embed_in_dwt_bands(payload_bits: str, bands: Dict[str, np.ndarray], 
    ...
    Args:
        ...
        Q_factor (float): Quantization factor (default 5.0). Higher = more capacity, lower PSNR
    ...
    # Use provided Q_factor for embedding
    # Must match extraction Q_factor for correct recovery!
    payload_bytes = len(payload_bits) // 8
    Q = Q_factor
...
def extract_from_dwt_bands(bands: Dict[str, np.ndarray], payload_bit_length: int,
    ...
    # Use provided Q_factor (MUST match embedding!)
    Q = Q_factor
