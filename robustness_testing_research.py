...
from a2_key_management import generate_key
...
class RobustnessTestingResearch:
    ...
    def embed_test_payload(self, cover_image_path, payload_text):
        ...
        # Process payload through CORRECT LayerX pipeline with Reed-Solomon ECC
        key = generate_key()
        ...
        # CRITICAL FIX: Add Reed-Solomon ECC protection
        payload_with_ecc = create_payload(payload_text.encode(), pickle.dumps(compression_table), compressed_payload)
        ...
        return stego_image, {
            ...
            "uses_reed_solomon": True  # Flag for correct pipeline
        }
    ...
    def test_extraction_after_modification(self, modified_stego_image, extraction_info):
        ...
        try:
            ...
            # CRITICAL FIX: Parse Reed-Solomon protected payload
            msg_len, tree_bytes, compressed_data = parse_payload(extracted_payload)
            compression_table = pickle.loads(tree_bytes)
            
            # Decompress and decrypt
            decompressed = decompress_huffman(compressed_data, compression_table)
